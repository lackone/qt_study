...
{
    int i;
    for (i = 0; i < 3; i++) slotAddEllipseItem();
    for (i = 0; i < 3; i++) slotAddPolygonItem();
    for (i = 0; i < 3; i++) slotAddTextItem();
    for (i = 0; i < 3; i++) slotAddRectItem();
    for (i = 0; i < 3; i++) slotAddAlphaItem();
    for (i = 0; i < 3; i++) slotAddFlashItem();
    for (i = 0; i < 3; i++) slotAddAnimationItem();
}
...
void MainWindow::slotAddTextItem()  //在场景中加入一个文字图元
{
    ...
    item->setFont(font);
    item->setFlag(QGraphicsItem::ItemIsMovable);
    item->setDefaultTextColor(QColor(rand() % 256, rand() % 256, rand() % 256));
    scene->addItem(item);
    item->setPos((rand() % int(scene->sceneRect().width())) - 200, (rand() % int(scene->sceneRect().height())) - 200);
}
